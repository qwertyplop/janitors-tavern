  ...
  return (
    <div className="bg-card border border-card-border rounded-xl p-6 space-y-5">
      <h2 className="text-lg font-semibold">
        {script.scriptName ? t(lang, 'editRegexScript', { name: script.scriptName }) : t(lang, 'newRegexScript')}
      </h2>

      <div className="space-y-1.5">
        <label className="text-xs font-medium text-muted-foreground">{t(lang, 'scriptName')}</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'findRegex')}</label>
          <input
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'replaceWith')}</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'applyToMessages')}</label>
          <div className="flex flex-wrap gap-2">
            {([0, 1] as const).map(num => {
              const label = num === 0 ? t(lang, 'placementUserInput') : t(lang, 'placementAiOutput');
              return (
                <button key={num} onClick={() => togglePlacement(num)}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'applyToRoles')}</label>
          <div className="flex flex-wrap gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'minDepth')}</label>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'maxDepth')}</label>
          <input type="number" className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm focus:outline-none focus:ring-1 focus:ring-ring"
            value={form.maxDepth ?? ''} onChange={e => set({ maxDepth: e.target.value === '' ? null : parseInt(e.target.value) })} placeholder="null" />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'order')}</label>
          <input type="number" className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm focus:outline-none focus:ring-1 focus:ring-ring"
            value={form.order} onChange={e => set({ order: parseInt(e.target.value) || 0 })} />
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'substituteMode')}</label>
          <select className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm focus:outline-none"
            ...
            <option value={0}>{t(lang, 'substituteNone')}</option>
            <option value={1}>{t(lang, 'substituteSubstitute')}</option>
            <option value={2}>{t(lang, 'substituteEscape')}</option>
          </select>
        </div>
      </div>

      <div className="space-y-2">
        <label className="text-xs font-medium text-muted-foreground">{t(lang, 'trimStrings')}</label>
        ...
        <div className="flex gap-2">
          <input className="flex-1 px-2.5 py-1.5 rounded bg-input border border-border text-foreground text-xs font-mono focus:outline-none focus:ring-1 focus:ring-ring"
            ...
            onKeyDown={e => e.key === 'Enter' && addTrim()} placeholder={t(lang, 'trimPlaceholder')} />
          <button onClick={addTrim} disabled={!trimInput.trim()} className="px-3 py-1.5 rounded bg-secondary text-secondary-foreground text-xs border border-secondary-border disabled:opacity-40">{t(lang, 'add')}</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        {([
          ...
          <div key={key} className="flex items-center gap-2">
            ...
            <label htmlFor={key} className="text-xs text-muted-foreground">{t(lang, labelKey)}</label>
          </div>
        ))}
      </div>

      <div className="border-t border-border pt-4 space-y-2">
        <label className="text-xs font-medium text-muted-foreground">{t(lang, 'preview')}</label>
        <div className="flex gap-2">
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-border">
        ...
        <button onClick={onCancel} className="px-4 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground">{t(lang, 'cancel')}</button>
...
function RegexScriptsTab() {
  ...
  const placementLabel = (p: number) => {
    const map: Record<number, string> = {
      0: t(lang, 'placementUserInput'),
      1: t(lang, 'placementAiOutput'),
      2: t(lang, 'placementSlashCommands'),
      3: t(lang, 'placementWorldInfo'),
    };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <p className="text-sm text-muted-foreground">{t(lang, 'regexScriptsSubtitle')}</p>
        {!showNew && !editing && (
          <div className="flex gap-2 shrink-0">
            ...
            {scripts.length > 0 && (
              <button onClick={handleExport}
                ...
                <Download size={14} /> {t(lang, 'export')}
              </button>
            )}
      ...
      {scripts.length === 0 && !showNew ? (
        <div className="text-center py-16 bg-card border border-card-border rounded-xl">
          ...
          <p className="text-muted-foreground text-sm">{t(lang, 'noRegexScripts')}</p>
          <p className="text-xs text-muted-foreground mt-1">{t(lang, 'regexScriptsSubtitle')}</p>
          <button onClick={() => setShowNew(true)} className="mt-4 text-primary text-sm hover:underline flex items-center gap-1 mx-auto">
            <Plus size={13} /> {t(lang, 'createNewScript')}
          </button>
        </div>
      ) : (
        <div className="space-y-2">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {script.disabled && <span className="text-xs bg-muted text-muted-foreground border border-border px-1.5 py-0.5 rounded">{t(lang, 'disabledBadge')}</span>}
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap text-xs text-muted-foreground">
                      <span>{t(lang, 'appliesTo', { placements: script.placement.map(p => placementLabel(p)).join(', ') })}</span>
                      {script.roles && script.roles.length > 0 && <><span>·</span><span>{t(lang, 'rolesLabel', { roles: script.roles.join(', ') })}</span></>}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => handleToggle(script.id)} className="p-2 rounded-lg hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors" title={script.disabled ? t(lang, 'enable') : t(lang, 'disable')}>
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <p className="text-sm text-muted-foreground">
          {t(lang, 'soSubtitle')} <span className="font-mono text-xs bg-muted/60 px-1 rounded">StructuredPrefill</span>.
        </p>
        <button
          ...
          {t(lang, 'slotReferenceBtn')} <ChevronDown size={12} className={cn('transition-transform', showDocs && 'rotate-180')} />
        </button>
      </div>
      ...
      <div className="bg-card border border-card-border rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between gap-3">
          <label className="text-xs font-medium text-muted-foreground">{t(lang, 'activePresetLabel')}</label>
        ...
        {presets.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground">{t(lang, 'noSOPresetsYet')}</p>
            <button onClick={() => setShowNewForm(true)} className="mt-2 text-primary text-xs hover:underline">{t(lang, 'createToGetStarted')}</button>
          </div>
        ) : (
          <div className="relative">
            <select
              ...
              <option value="" disabled>{t(lang, 'selectPresetPlaceholder')}</option>
              {presets.map(p => (
      ...
      {form && (
        <>
          <div className="bg-card border border-card-border rounded-xl p-5 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleDelete}
                ...
                {deleteConfirm ? t(lang, 'confirmDeletePreset') : t(lang, 'deletePreset')}
              </button>
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">{t(lang, 'presetNameLabel')}</label>
              <input className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm focus:outline-none focus:ring-1 focus:ring-ring"
            ...
            <div className="space-y-4 border-t border-border pt-4">
              <ToggleSwitch
                ...
                label={t(lang, 'enableSOLabel')}
                description={t(lang, 'enableSODesc')}
              />
              <ToggleSwitch
                checked={form.hidePrefillInDisplay}
                onChange={v => setField('hidePrefillInDisplay', v)}
                label={t(lang, 'hidePrefillLabel')}
                description={t(lang, 'hidePrefillDesc')}
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-border pt-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">{t(lang, 'minCharsAfterPrefixLabel')}</label>
                ...
                <p className="text-xs text-muted-foreground">{t(lang, 'minCharsAfterPrefixDesc')}</p>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">{t(lang, 'newlineTokenLabel')}</label>
                <input className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm font-mono focus:outline-none focus:ring-1 focus:ring-ring"
                  value={form.newlineToken}
                  onChange={e => setField('newlineToken', e.target.value)}
                  placeholder="\n" />
                <p className="text-xs text-muted-foreground">{t(lang, 'newlineTokenDesc')}</p>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">{t(lang, 'continueOverlapLabel')}</label>
                <input type="number" min={0}
                  className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm focus:outline-none focus:ring-1 focus:ring-ring"
                  value={form.continueOverlapChars}
                  onChange={e => setField('continueOverlapChars', Math.max(0, parseInt(e.target.value) || 0))} />
                <p className="text-xs text-muted-foreground">{t(lang, 'continueOverlapDesc')}</p>
              </div>
            </div>

            <div className="space-y-1.5 border-t border-border pt-4">
              <label className="text-xs font-medium text-muted-foreground">{t(lang, 'antiSlopLabel')}</label>
              <textarea
                ...
                placeholder={"word1\nword2\nphrase to ban"}
              />
              <p className="text-xs text-muted-foreground">{t(lang, 'antiSlopDesc')}</p>
            </div>
            ...
            <div className="space-y-3 border-t border-border pt-4">
              <ToggleSwitch
                ...
                label={t(lang, 'overridePrefillLabel')}
                description={t(lang, 'overridePrefillDesc')}
              />
              {form.overridePrefillEnabled && (
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">{t(lang, 'prefillTemplateLabel')}</label>
                  <textarea
                    className="w-full px-3 py-2 rounded-lg bg-input border border-border text-foreground text-sm font-mono focus:outline-none focus:ring-1 focus:ring-ring resize-none"
                    rows={6}
          ...
          <div className="flex items-center justify-end gap-3">
            {saveStatus === 'saved' && <span className="text-xs text-green-500">{t(lang, 'savedAndApplied')}</span>}
            {saveStatus === 'error' && <span className="text-xs text-destructive">{t(lang, 'failedToApply')}</span>}
            <button
              onClick={handleSave}
              disabled={saveStatus === 'saving'}
              className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 disabled:opacity-50">
              {saveStatus === 'saving' ? t(lang, 'saving') : t(lang, 'saveAndApply')}
            </button>
          </div>
        </>
      )}
